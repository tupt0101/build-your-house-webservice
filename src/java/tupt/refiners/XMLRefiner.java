...
import static tupt.constants.XMLRefineConstant.*;
import tupt.constants.CharacterConstant;
import tupt.utils.CharacterUtil;
...
public class XMLRefiner implements Serializable {
    ...
    public String doRefine(String source) {
        ...
        if (CONTENT.equals(state)) {
            String parsedEntity = Jsoup.parse(content.toString()).text().trim().replaceAll("&", "&amp;");
    ...
    private void handleStateEqual(char character) {
        if (Character.isSpaceChar(character)) {
            // do nothing
        } else if (character == CharacterConstant.DOUBLE_QUOTE || character == CharacterConstant.SINGLE_QUOTE) {
            state = ATTR_VALUE_Q;
            quote = character;
